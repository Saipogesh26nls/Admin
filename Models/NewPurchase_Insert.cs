...

{
    public class NewPurchase_Insert
    {
        ...
        public List<New_Purchase> Pcode_to_PartNo(string data)
        {
            ...
            SqlConnection Con = new SqlConnection(ConfigurationManager.ConnectionStrings["geriahco_db"].ConnectionString);
            Con.Open();
            string cmd1 = "select P_Part_No,P_Description,P_Closing_Balance from Product_Master where P_code = '" + data + "'";
            SqlCommand SqlCmd1 = new SqlCommand(cmd1, Con);
            SqlDataReader dr = SqlCmd1.ExecuteReader();
            while (dr.Read())
            {
                ItemQm.Add(new New_Purchase
                {
                    P_Part_No = dr["P_Part_No"].ToString(),
                    P_Description = dr["P_Description"].ToString(),
                    P_Close_Bal = Convert.ToInt32(dr["P_Closing_Balance"])
                }
                );
            }
            Con.Close();
    ...
    public class Goods_RI
    {
        ...
        public List<GoodsRI> PM_list(string package, string value, string partno, string descp)
        {
            ...
            {
                string cmd1 = "SELECT * FROM Product_Master WHERE P_Package LIKE '%" + package + "%' and P_Part_No Like '%" + partno + "%'";
                SqlCommand SqlCmd1 = new SqlCommand(cmd1, Con);
                SqlDataReader dr = SqlCmd1.ExecuteReader();
                while (dr.Read())
                {
            ...
            {
                string cmd1 = "SELECT * FROM Product_Master WHERE P_Value LIKE '%" + value + "%' and P_Part_No Like '%" + partno + "%'";
                SqlCommand SqlCmd1 = new SqlCommand(cmd1, Con);
                SqlDataReader dr = SqlCmd1.ExecuteReader();
                while (dr.Read())
                {
